...
func Initialize() *http.ServeMux {
	...
	mux.Handle("PUT /password", utils.Handler(mw.Auth, mw.RenewUpdateToken, handlers.UpdatePassword))

	mux.Handle("PUT /user", utils.Handler(mw.Auth, mw.RenewUpdateToken, handlers.UpdateUser))

	mux.Handle("GET /adv/{id}", utils.Handler(handlers.GetAdv))
	mux.Handle("GET /adv", utils.Handler(handlers.GetAdvList))

	mux.Handle("GET /my/adv/{id}", utils.Handler(mw.Auth, mw.RenewUpdateToken, handlers.GetUsersAdv))
	mux.Handle("GET /my/adv", utils.Handler(mw.Auth, mw.RenewUpdateToken, handlers.GetUsersAdvList))

	mux.Handle("POST /adv", utils.Handler(mw.Auth, mw.RenewUpdateToken, handlers.CreateAdv))
	mux.Handle("PUT /adv/{id}", utils.Handler(mw.Auth, mw.RenewUpdateToken, handlers.UpdateAdv))
	mux.Handle("DELETE /adv/{id}", utils.Handler(mw.Auth, mw.RenewUpdateToken, handlers.DeleteAdv))
